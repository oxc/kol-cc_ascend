...
generic_t zone_available(location loc)
{
	...
	case $location[The Tunnel of L.O.V.E.]:
		retval._boolean = get_property("loveTunnelAvailable").to_boolean() && !get_property("_loveTunnelUsed").to_boolean();
		break;
	case $location[The Neverending Party]:
		retval._boolean = get_property("neverendingPartyAlways").to_boolean() || get_property("_neverendingPartyToday").to_boolean();
		break;
	...
	case $location[The Canadian Wildlife Preserve]:
		if(to_int(today_to_string()) < 20190000)
		{
			retval._boolean = true;
		}
		break;
